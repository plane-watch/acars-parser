// Package label27 parses Label 27 position report messages.
package label27
...
// Result represents a position report from label 27 messages.
type Result struct {
	MsgID       int64   `json:"message_id"`
	Timestamp   string  `json:"timestamp"`
	Tail        string  `json:"tail,omitempty"`
	MsgType     string  `json:"msg_type"`
	Format      string  `json:"format,omitempty"`         // POS01, POS02, etc.
...
// Parser parses Label 27 position messages.
type Parser struct{}

var (
	// Match POS01AFL1866 format (with flight level)
	posAFLRe = regexp.MustCompile(`(?i)^(POS\d+)AFL(\d+)`)

	// Match POS01SU0245 or POS01SDM6599 format (with flight number, 2-3 letter airline code)
	posFlightRe = regexp.MustCompile(`(?i)^(POS\d+)([A-Z]{2,3}\d+)`)
	...
	// Match TEMP- 32 or TEMP -32 or TEMP-32
...
func (p *Parser) Name() string     { return "label27" }
func (p *Parser) Labels() []string { return []string{"27"} }
func (p *Parser) Priority() int    { return 100 }

func (p *Parser) QuickCheck(text string) bool {
	upper := strings.ToUpper(text)
	// Check for POS format identifier at start
	return strings.HasPrefix(upper, "POS0") || strings.HasPrefix(upper, "POS1")
}
...
func (p *Parser) Parse(msg *acars.Message) registry.Result {
	...
	// Parse ETA
	if m := etaRe.FindStringSubmatch(upper); m != nil {
		etaStr := m[1]
		if len(etaStr) == 4 {
			hh := etaStr[0:2]
			mm := etaStr[2:4]
	...
	keywords := map[string]bool{
		"POS": true, "AFL": true, "FUEL": true, "TEMP": true,
		"WDIR": true, "WSPD": true, "LATN": true, "LATS": true,
		"LONE": true, "LONW": true, "ETA": true, "ALT": true, "TUR": true,
	}
